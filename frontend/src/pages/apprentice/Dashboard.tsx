...
const ApprenticeDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-8">
      {/* Hero Section with Green Gradient - Matching Sidebar */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 max-w-2xl">
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 border border-white/20">
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <span className="text-xs sm:text-sm text-green-100">{t('Tasdiqlangan', language)}</span>
                  </div>
                  <p className="text-2xl sm:text-3xl font-bold">{approvedTasks.length}</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 border border-white/20">
                  <div className="flex items-center space-x-2 mb-1">
                    <Activity className="h-4 w-4 sm:h-5 sm:w-5 text-emerald-300" />
                    <span className="text-xs sm:text-sm text-green-100">{t('Daromad', language)}</span>
                  </div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 border border-white/20">
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <span className="text-xs sm:text-sm text-green-100">{t('Reyting', language)}</span>
                  </div>
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between mb-4">
                  <div className={`flex h-14 w-14 items-center justify-center rounded-xl ${stat.iconBg} group-hover:scale-110 transition-transform duration-300`}>
        ...
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 p-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-2xl font-bold">{t('Daromad tarixi', language)}</h3>
                <p className="text-green-100">{t('Tasdiqlangan vazifalardan olingan pullar', language)}</p>
              </div>
            </div>
            <div className="text-right">
              <div className="text-3xl font-bold">{new Intl.NumberFormat('uz-UZ').format(user?.earnings || 0)}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border-2 border-green-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-green-700 mb-1">{t('Jami daromad', language)}</p>
            ...
            <div className="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-900">{approvedTasks.length}</p>
          ...
          <div className="space-y-3">
            ...
            {approvedTasks.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">{t('Hali daromad yo\'q', language)}</p>
                <p className="text-sm text-gray-400 mt-1">{t('Vazifalarni bajaring va daromad oling!', language)}</p>
              </div>
            ) : (
              <div className="space-y-3 max-h-80 overflow-y-auto">
                {approvedTasks
                  ...
                  .map((task: any, index: number) => (
                    <div key={task._id} className="flex items-center justify-between p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200 hover:shadow-md transition-all duration-300">
                      <div className="flex items-center gap-4 flex-1">
                        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-green-500 text-white font-bold text-sm">
                          {index + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                          <h5 className="font-semibold text-gray-900 truncate">{task.title}</h5>
      ...
      {/* Bugungi vazifalar - Priority Section with Green Theme */}
      <div className="card p-0 overflow-hidden shadow-xl border-2 border-green-100">
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 p-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-2xl font-bold">{t('Bugungi vazifalar', language)}</h3>
                <p className="text-green-100">{t('Bugun', language)} {todayTasks.length} {t('ta vazifa', language)}</p>
              </div>
            </div>
            {todayTasks.length > 0 && (
              <div className="text-right">
        ...
        <div className="p-6">
          {todayTasks.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">{t('Bugun sizga vazifa berilmagan yoki barchasi bajarilgan', language)}</p>
            </div>
          ) : (
            <div className="space-y-4">
              {todayTasks.map((task: any, index: number) => {
                ...
                if (task.status === 'completed' || task.status === 'approved') {
                  ...
                  statusIcon = <Zap className="h-5 w-5 text-blue-600" />;
                }
                
                return (
                  <div 
                    key={task._id} 
                    className={`border-l-4 ${borderColor} ${bgColor} rounded-lg p-5 transition-all duration-300 hover:shadow-md group`}
                    style={{ animationDelay: `${index * 50}ms` }}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-4 flex-1">
                        <div className={`mt-1 p-2 rounded-lg ${task.status === 'completed' || task.status === 'approved' ? 'bg-green-100' : task.status === 'in-progress' ? 'bg-yellow-100' : 'bg-blue-100'}`}>
                          {statusIcon}
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mb-3">
                            <span className={`inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full ${getPriorityColor(task.priority)}`}>
                            ...
                            <span className={`inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full ${getStatusColor(task.status)}`}>
                      ...
                      <div className="flex flex-col space-y-2 ml-4">
                        {task.status === 'assigned' && (
                          <button 
                            ...
                            className="btn-sm bg-gradient-to-r from-green-600 to-emerald-600 text-white hover:from-green-700 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl transition-all duration-300 flex items-center space-x-2 whitespace-nowrap"
                          >
                        ...
                        {(task.status === 'completed' || task.status === 'approved') && (
                          <div className="flex items-center text-green-600 font-semibold text-sm bg-green-50 px-4 py-2 rounded-lg">
          ...
          <div className="bg-gradient-to-r from-blue-500 to-cyan-500 p-5 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <h3 className="text-lg font-bold">{t('Faol vazifalar', language)}</h3>
              </div>
          ...
          <div className="p-5">
            {[...assignedTasks, ...inProgressTasks].length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">{t('Faol vazifalar yo\'q', language)}</p>
              </div>
            ) : (
              <div className="space-y-3">
                {[...assignedTasks, ...inProgressTasks].slice(0, 5).map((task: any) => (
                  <div key={task._id} className="group flex items-center justify-between p-4 border-2 border-gray-100 rounded-xl hover:border-blue-300 hover:bg-blue-50 transition-all duration-300 cursor-pointer">
                    ...
                    <div className="flex items-center space-x-2 ml-3">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-lg ${getPriorityColor(task.priority)} flex items-center`}>
        ...
        <div className="card p-0 overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
          <div className="bg-gradient-to-r from-green-500 to-emerald-500 p-5 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <h3 className="text-lg font-bold">{t('Bajarilgan ishlar', language)}</h3>
              </div>
              <span className="px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-sm font-semibold">
          ...
          <div className="p-5">
            {[...completedTasks, ...approvedTasks].length === 0 ? (
              <div className="text-center py-8">
                <Award className="h-12 w-12 text-gray-300 mx-auto mb-3" />
                <p className="text-gray-500">{t('Hali bajarilgan ishlar yo\'q', language)}</p>
              </div>
            ) : (
              <div className="space-y-3">
                {[...completedTasks, ...approvedTasks].slice(0, 5).map((task: any) => (
                  <div key={task._id} className="group flex items-center justify-between p-4 border-2 border-gray-100 rounded-xl hover:border-green-300 hover:bg-green-50 transition-all duration-300 cursor-pointer">
                    <div className="flex items-center space-x-3 flex-1 min-w-0">
                      <div className={`flex h-10 w-10 items-center justify-center rounded-xl flex-shrink-0 ${
                    ...
                    <div className="flex items-center space-x-2 ml-3">
                      <span className={`px-3 py-1 text-xs font-semibold rounded-lg ${getStatusColor(task.status)} flex items-center gap-1`}>
